...
function Home() {
  ...
  const orderId = queryParams.get("orderId"); // Get order ID from URL
  const itemID = queryParams.get("itemID"); // Get order ID from URL
  useEffect(() => {
    if (success === "true" && orderId) {
      // Update order status to "success" in Firebase
      const orderRef = doc(db, "payments", orderId);
      const orderRefR = doc(db, "Requests", itemID);
      updateDoc(orderRef, { status: "success" })
      updateDoc(orderRefR, { status: "success" })
    } else {
      // setStatus("❌ Payment Failed");
    }
  }, [success, orderId ,itemID]);
    ...
    <main className='space-y-20'>
      <section className='grid grid-cols-1 lg:grid-cols-2 gap-2 md:gap-12 md:px-[100px] px-10'>
        <article data-aos="fade-right" data-aos-duration="1000" data-aos-delay="1000" className='container flex flex-col justify-center space-y-8'>
          ...
          <div className="flex md:justify-between flex-col md:flex-row items-center gap-4 text-xs md:text-xl">
            <button onClick={() => { navigate('/bookTest', { state: { option: 'Free Session' } }); window.scroll(0, 0) }} className={`p-4 sm:px-6 w-[150px] md:w-[250px] rounded-3xl bg-[var(--Yellow)] ${i18n.language == 'ar' && "px-8"}`}>{t('bookFreeTrial')}</button>
            <button onClick={() => { navigate('/courses'); window.scroll(0, 0) }} className='p-4 sm:px-6 w-[150px] md:w-[250px] rounded-3xl border-2 border-[var(--Yellow)]'>{t('exploreCourses')}</button>
          </div>
        </article>
      ...
      <section className='px-10 md:px-12 lg:px-60'>
        ...
        <article className='grid grid-cols-1 gap-4 lg:grid-cols-8 md:gap-8 relative'>
          ...
          <div data-aos="fade-right" data-aos-duration="3000" data-aos-delay="1000" className='border-2 border-[var(--SubTextBorder)] text-start rounded-4xl px-3 py-3 md:px-5 md:py-14 space-y-6 md:space-y-10 z-0 md:col-span-4'>
            <h3 className='font-semibold text-2xl md:text-xl md:w-72'>{t('firstStep1')} <span className="text-[var(--Yellow)]">{t('firstStep2')}</span> {t('firstStep3')}</h3>
            <p className='text-sm md:text-lg md:w-72'>{t('freeSession')}</p>
          </div>

          <div className='hidden lg:block'></div>

          <img src={offer} className={`absolute hidden lg:block w-72 h-72 z-10 end-full md:end-[31%]`} alt="offer" />
        ...
        <div data-aos="fade-left" data-aos-duration="3000" data-aos-delay="1000" className="flex justify-end my-8">
          <button onClick={() => { navigate('/bookTest', { state: { option: "Free Session" } }); window.scroll(0, 0) }} className={`p-3 sm:p-4 rounded-3xl bg-[var(--Yellow)] ${i18n.language == 'ar' ? 'w-[150px]' : 'w-[200px]'} text-sm sm:text-base`}>
            {t('bookFreeTrial')}
          </button>
        </div>
      </section>
