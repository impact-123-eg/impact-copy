...
function HomePage() {
 ...
 paid.filter(paid => paid?.status === "success").forEach(payment => {
    if (payment.Date) {
...
const dataChart = {
 ...
 const details = [
  ...
  { number: freeTest.length +freeSession.length + paid.filter(item => item.status === "success").length, description: 'Total Student' },
   ...
   <section className="grid grid-cols-2 space-x-10 rounded-2xl p-10 border-2 border-[var(--SubTextBorder)]/50">
    <article className="space-y-8 flex flex-col h-full">
     ...
     <div className="mt-auto justify-self-end space-y-2">
      <h1 className="text-3xl text-[var(--Yellow)] font-bold">{paid.filter(payment => payment.status === "success").length} Students</h1>
      <p>{year}</p>
     </div>
    </article>
   ...
   <section className="space-y-10">
    ...
    <div className="overflow-hidden border-2 border-[#347792] rounded-xl">
     <table className="w-full text-center table-auto">
      ...
      <tbody>
       {requests.filter((e)=>e.status === "success").slice(0,3).map((req) => (
